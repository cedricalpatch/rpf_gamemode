...
vRPclient = Tunnel.getInterface("vRP","vf_lscustoms")
--[[
...
RegisterServerEvent("LSC:buttonSelected")
AddEventHandler("LSC:buttonSelected", function(name, button)
	--local src = source --Just so you can buy everything while there is no money system implemented
	if button.price then -- check if button have price
    ...
    --if vRP.hasGroup({user_id,'citizen'}) then
		...
		--if user.cash >= tonumber(button.price) then
          --          TriggerEvent('vf_base:ClearCash', src, button.price)
            --Db.UpdateCash(player, -button.price, function()
			TriggerClientEvent("LSC:buttonSelected", source,name, button, true)
			--end)
			--mymoney  = mymoney - button.price
		else
		--vRPclient.notify(player,{"~g~Payement : ~w~Tu as pas assee de liquide !"})
			--TriggerClientEvent("fs_freemode:notify", "CHAR_BANK_FLEECA", 3, 20, "Payement au Grarage", "Impossible !", "Tu as pas assee de liquide !")
