...
AddEventHandler('lsv:cratePickedUp', function()
	...
	logger:Info('Picked up by user_id { '..user_id..' }')

	vRP.giveMoney({user_id, Settings.crateDropSettings.cash})
		crateDropData = nil
		eventFinishedTime = GetGameTimer()
		TriggerClientEvent('lsv:removeCrate', -1, user_id, Settings.crateDropSettings.weaponClipCount, Settings.crateDropSettings.cash)
end)
