...
AddTextEntry("XNOV_ENTER", "Press ~INPUT_CONTEXT~ Pour loue une place de parking Pour ~g~2000€~w~. Vien avant avec ton vehicule pour pas perdre ta place !")
...
Citizen.CreateThread(function()
    ...
    for ln,loc in pairs(xnGarageConfig.locations) do
        ...
        BeginTextCommandSetBlipName("STRING")
    	      AddTextComponentString(xnGarageConfig.GroupMapBlips and "Parking" or ln)
    ...
    Citizen.CreateThread(function()
        ...
        if wait then
            BeginTextCommandBusyString("STRING")
                AddTextComponentSubstringPlayerName("Chargement Parking")
...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('vmm', 'Vos Vehicules')
        WarMenu.CreateSubMenu('vmm:veh', 'vmm', 'Organiser')
            WarMenu.CreateSubMenu('vmm:move', 'vmm:veh', 'Ou ?')
            WarMenu.CreateSubMenu('vmm:delete', 'vmm:veh', 'Tu es sur ?')

	WarMenu.SetSubTitle('vmm', "Vos Vehicules")
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('mech', 'Parking')
        WarMenu.CreateSubMenu('mech:location', 'mech', 'Vehicules')

	WarMenu.SetSubTitle('mech', "Vos Vehicules")
	...
	while true do
        ...
        elseif IsControlJustReleased(0, 65) then
            vehicleTable = false
            TriggerServerEvent("xnov:reqVehicles")
            while not vehicleTable do Citizen.Wait(0) end
                ...
                if Vdist2(GetEntityCoords(ent),ix,iy,iz) < location.inLocation[2]*2.5 and not IsPedSprinting(GetPlayerPed(-1)) then
                    if not allowed then
                        ...
                        if IsControlJustReleased(0, 51) then
                            TriggerServerEvent('business:checkmoney9')
...
Citizen.CreateThread(function()
    if xnGarageConfig.EnableInteriors then
        ...
        RequestIpl("ex_exec_warehouse_placement ")
        --EnableInteriorProp(256513, "Basic_style_set")
        ...
        EnableInteriorProp(-744024657, "xs_arenalights_track_dyst03")
        EnableInteriorProp(-744024657, "xs_arenalights_track_midday")
        EnableInteriorProp(-1076217392, "xs_combined2_dyst_08_pipes_02")
        EnableInteriorProp(2007005006, "xs_combined2_dyst_longbuild_a_09")
        RefreshInterior(-744024657)
    end
end)
...
AddEventHandler('business:notenough', function()
    vRP.notifyPicture({"~r~Not enough money."})
end)
