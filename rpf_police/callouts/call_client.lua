...
AddEventHandler('pis:shoplifting', function()
	...
	local ClerkPeaceQuotes = {"Reviens ici!", "Officier! C'est le gars", "Il a pris quelque chose!"}
	local ClerkAgressiveQuotes = {"Reviens ici, enculé !!", "Officier! C'est l'enculé", "Cet enfoiré a pris ma merde!", "Enfoiré, récupère-moi!", "Tirez sur cette chienne!", "Je vais le tuer, putain!"}
...
AddEventHandler('pis:weapon:face:q', function()
	...
	if speech == "Normal" then
		local officerFaceQuotes = {"Fais le tour!", "Face à moi!"}
		ShowNotification("~o~Officer: ~w~" .. officerFaceQuotes[math.random(#officerFaceQuotes)])
	else
		local officerFaceQuotes = {"Tournez la gueule!", "Fais-moi face grosse merde!"}
		ShowNotification("~o~Officer: ~w~" .. officerFaceQuotes[math.random(#officerFaceQuotes)])
	end
...
AddEventHandler('pis:notification', function()
	ShowNotification("~w~AA toutes les unités, répondez!" .. response .. " à " .. location .. " pour " .. callout)
end)
